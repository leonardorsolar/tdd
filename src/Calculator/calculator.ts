export class Calculator {
  ...
  private hasNonNumericCharacters(addendsArray: string[]): boolean {
    const found = addendsArray.find(
      addend => !this.isNumber(addend) && !this.isEmptyString(addend),
    );
    return !!found;
  }
  ...
  public add(numbers: string): number {
    ...
    if (this.hasConsecutiveComments(numbers)) throw new Error('Invalid format');
